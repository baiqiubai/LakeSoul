...
public class CompactionBroadcastProcessFunction extends KeyedBroadcastProcessFunction<
            ...
            if (compaction != null) {
                ...
                if (valueTimestamp < compactTimstamp && currTimestamp - valueTimestamp > expiredTime){
                    ...
                    try (Connection connection = dataSource.getConnection()) {
                        ...
                        if( index == 0 ){
                            long syncTimestamp = Math.min(compactTimstamp, currTimestamp - expiredTime);
                            log.info("清除PGMQ timestamp位于 " +  syncTimestamp + "之前的数据");
                            tryCleanPGMQ(value.tableId, syncTimestamp, connection);
                        }
                    } catch (Exception e) {
        ...
        if (compactionOut != null) {
            ...
            if (eventTimestamp < compactTimestamp && timestamp - eventTimestamp > expiredTime){
                ...
                try (Connection connection = dataSource.getConnection()) {
                    ...
                    int index = getRuntimeContext().getTaskInfo().getIndexOfThisSubtask();
                    if (index == 0) { 
                        long syncTimestamp = Math.min(compactTimestamp, timestamp - expiredTime);
                        log.info("清除PGMQ timestamp位于 " +  syncTimestamp + "之前的数据");
                        tryCleanPGMQ(value.tableId, syncTimestamp , connection);
                    }
                    timerTsState.clear();
    ...
    private void tryCleanPGMQ(String tableId, long expiredThreshold, Connection connection) {
            cleanUtils.cleanPGMQ(tableId, connection);
    }
}